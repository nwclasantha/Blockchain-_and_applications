...
# === BLOCKCHAIN CLASS (With Consensus-Based Attack Simulations) ===
class Blockchain:
    ...
    # === CONSENSUS-BASED ATTACK SIMULATIONS ===
    ...
    def selfish_mining_attack(self):
        ...
        hidden_blocks = random.randint(1, 3)
        for _ in range(hidden_blocks):
            self.chain.pop()  # Simulate miners withholding blocks
        print(f"⚠️ {hidden_blocks} blocks were secretly mined but not broadcasted!")

    def block_withholding_attack(self):
        print("\n🚨 Simulating Block Withholding Attack...")
        attacker = "MaliciousMiner"
        print(f"⚠️ {attacker} mines a valid block but refuses to share it with the network!")

    def fork_after_withholding_attack(self):
        print("\n🚨 Simulating Fork After Withholding Attack...")
        malicious_chain = self.chain[:len(self.chain)//2]
        print(f"⚠️ Attackers create a forked blockchain with {len(malicious_chain)} blocks!")
...
def visualize_blockchain(blockchain):
    ...
    plt.title("Blockchain Security - Consensus Attack Simulations")
    plt.show()
...
# Step 3: Simulate Consensus-Based Attacks
blockchain.fifty_one_percent_attack()
blockchain.double_spending_attack("Eve", "Mallory")
blockchain.selfish_mining_attack()
blockchain.block_withholding_attack()
blockchain.fork_after_withholding_attack()
